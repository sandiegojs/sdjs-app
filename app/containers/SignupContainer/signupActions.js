...
export function submitSignUp(credentials, navigate) {
	return {
		type: 'SUBMIT_SIGN_UP',
		payload: axios
			.post('http://f5f990cc.ngrok.io/api/users', credentials)
			.then(response => {
				...
				if (statusCode.test(signUpRes)) {
					...
					return axios
						.post('http://f5f990cc.ngrok.io/api/users/login', {email, password})
						.then(r => r.data);
				}
			})
			.catch(error => {
...
export function thirdPartyLogin(loginObj) {
	return {
		...
		payload: axios
			.post('http://f5f990cc.ngrok.io/loginthirdparty', loginObj)
