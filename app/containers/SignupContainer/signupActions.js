...
export function submitSignUp(credentials, navigate) {
	return {
		type: 'SUBMIT_SIGN_UP',
		payload: axios
			.post('https://7f29f798.ngrok.io/api/users', credentials)
			.then(response => {
				...
				if (statusCode.test(signUpRes)) {
					...
					return axios
						.post('https://7f29f798.ngrok.io/api/users/login', {email, password})
						.then(r => r.data);
				}
			})
			.catch(error => {
...
export function thirdPartyLogin(loginObj) {
	return {
		...
		payload: axios
			.post('https://7f29f798.ngrok.io/loginthirdparty', loginObj)
